...
mod test {
    ...
    impl BdkCli {
        ...
        fn new(
            ...
            println!("Building app with features : {}", feat);
        ...
        fn wallet_exec(&self, args: &[&str]) -> Result<Value, IntTestError> {
            let datadir = self.node_datadir.as_os_str().to_str().unwrap();
            let mut wallet_args =
        ...
        fn node_exec(&self, args: &[&str]) -> Result<Value, IntTestError> {
            let datadir_path = self.node_datadir.as_os_str().to_str().unwrap();
            let mut node_args = ["--datadir", datadir_path, "node"].to_vec();
            for arg in args {
                node_args.push(arg);
            }
    ...
    fn basic_wallet_ops(feature: &str) {
        // Create a temp file for testing env
        let temp = TempDir::new().unwrap().path().to_path_buf();
        // Create bdk-cli instance
        let bdk_cli = BdkCli::new("regtest", temp, false, &[feature]).unwrap();
