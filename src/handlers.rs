        ...
        CreateTx {
            ...
            if send_all {
                ...
                for recipient in dns_recipients {
                    ...
                    let (resolver, instructions) = parse_dns_instructions(&recipient.0, cli_opts.network, Some(dns_resolver.clone()))
                        .await
...
pub(crate) async fn handle_command(cli_opts: CliOpts) -> Result<String, Error> {
    ...
    let result: Result<String, Error> = match subcommand {
        ...
        CliSubCommand::ResolveDnsRecipient { hrn, resolver } => {
            let res = handle_resolve_dns_recipient_command(cli_opts.pretty, &hrn, resolver, cli_opts.network)
                .await?;
            Ok(res)
        }
    };
